...
def transform_poses_pca_return_scale(poses: np.ndarray) -> Tuple[np.ndarray, np.ndarray, float]:    #ii       dublicated the whole function
  """Transforms poses so principal components lie on XYZ axes.
  ...
  Returns:
    A tuple (poses, transform, scale_factor), with the transformed poses and the applied
    camera_to_world transforms and the factor calculated for having all poses in the unit sphere.
  """
  t = poses[:, :3, 3]
  t_mean = t.mean(axis=0)
  t = t - t_mean
  ...
  return poses_recentered, transform, scale_factor                                                 #ii
